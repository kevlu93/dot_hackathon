...
fn main() {
    ...
    daily_holdings.get("ACA").unwrap().iter().zip(daily_holdings_mv.get("ACA").unwrap()).for_each(|(x, y)| println!("daily holdings {}: {}, {}", x.0, x.1, y.1));
    transactions_mv.get("ACA").unwrap().iter().for_each(|x| println!("transaction {}: {}", x.0, x.1));
    ...
    for token in transactions.keys() {
        ...
        println!("mv vec: {:?}", &mv_vec[..]);
        println!("mv vec: {:?}", &date_vec[..]);
    ...
    loop {
        terminal.draw(|f| {
            ...
            let datapoints: Vec<(f64, f64)> = daily_holdings_mv.get(**selected_token).unwrap().values().enumerate().map(|(x,y)|(0.0 + x as f64, *y)).collect();
            let datasets = vec![
                Dataset::default()
